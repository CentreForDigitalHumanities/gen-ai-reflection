...
def get_challenges_opportunities_per_di(
    category: ChallengeOpportunity.Category,
) -> dict[str, list[str]]:
    """Return a dict of Dublin indicators and their corresponding challenges or opportunities."""
    entries = list(
        ChallengeOpportunity.objects.filter(category=category).values(
            "dublin_indicator", "text"
        )
    )
    return {
    ...
    return Response(
        {
            ...
            "challenges": get_challenges_opportunities_per_di(
                ChallengeOpportunity.Category.CHALLENGE
            ),
