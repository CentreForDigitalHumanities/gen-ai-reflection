...
class AdjustmentAdmin(ListDescriptionMixin, SingleRowTextareaMixin, admin.ModelAdmin):
    ...
    list_description = "These adjustments are shown in Step 2 ('Assessments') after a user answers 'Yes' to the question whether the use of GenAI affects the extent to which the associated learning outcome can be tested."
...
class AssessmentFormAdmin(ListDescriptionMixin, admin.ModelAdmin):
    ...
    list_description = (
        "These assessment forms are selectable in the dropdown menu on Step 2 ('Assessments') of the application. "
...
class UseExampleAdmin(ListDescriptionMixin, SingleRowTextareaMixin, admin.ModelAdmin):
    ...
    list_description = "These examples appear in Step 3 ('Rethinking courses in times of GenAI'). They are shown if the associated AI Assessment scale level is selected."
...
class KnownAiUseAdmin(ListDescriptionMixin, SingleRowTextareaMixin, admin.ModelAdmin):
    ...
    list_description = "Known AI uses are selectable in Step 2 ('Assessments') of the application. They are shown when the user selects the associated assessment. A user can select a Known AI use to see related examples. Make sure to add these as well."
