...
def main(argv):
    ...
    print(activate_venv)
    if not (pip_tools and backpack and frontpack and funcpack):
...
def activate_frontend():
    framework = "angular"
    include_authentication = "No, thank you." == "Yes, please!"
    ...
    if framework == "backbone":
        ...
        project_name = "gen_ai_reflection".replace("_", "-")
        Command("Install dependencies", ["yarn", "install", "--ignore-scripts"])()
        Command(
            "Creating project",
            [
                "yarn",
        ...
        Command("Creating localizations", ["yarn", "i18n"], cwd="frontend")()
        for lang in "en:English,nl:Nederlands".split(","):
            ...
            if code != "en":
                with open(f"frontend/locale/messages.{code}.xlf", "w") as file:
                    ...
                    targeted = re.sub(
                        r'(source-language="[^"]+"[^>]*)',
                        f'\\g<1> target-language="{code}"',
                        messages,
                    )
                    try:
                        with open(
                            f"frontend/locale/messages.{code}.json", "r"
                        ) as pretranslated:
        ...
        if "4200" != "4200":
            Command(
                ...
                [
                    ...
                    "ng",
                    "config",
                    "projects.gen-ai-reflection.architect.serve.options.port",
                    "4200",
                ],
                cwd="frontend",
            )()
